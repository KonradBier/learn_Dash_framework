...
def displayClick(btn1, btn2, btn3):
    if int(btn1) > int(btn2) and int(btn1) > int(btn3):
        msg = 'Button1 został wcisnięty jako ostatni'
    elif int(btn2) > int(btn1) and int(btn2) > int(btn3):
        msg = 'Button2 został wcisnięty jako ostatni'
    elif int(btn3) > int(btn1) and int(btn3) > int(btn2):
        msg = 'Button3 został wcisnięty jako ostatni'
    else:
        msg = 'Żaden przycisk nie został wcisnięty'
    return html.Div([
